	...
	var planets = [Mercury, Earth, Jupiter, Saturn, Uranus, Neptune, Halley];
		...
		for(var i=0; i<planets.length; i++) {
			(function(i, planets, scene) {
				...
				var heliocentricObj		= $("<a></a>")
					...
					.click(function() {
						scene.setView(planetName, function() {
							var dialog = new DialogWindow(scene, "objectInfo", planetInfo, planetName, "Sun");
					...
					satelliteObj.html(
						...
						.click(function() {
							scene.setView(satelliteName, function() {
								var dialog = new DialogWindow(scene, "objectInfo", satelliteInfo, satelliteName, planetName);
				...
				self.dialog = new DialogWindow(scene, "tour", accordionNav, "Tour", "Sun", 400);
			}(i, planets, scene));
		}
	};
