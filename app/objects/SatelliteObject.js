...
function ($, Mesh, Astrodynamics, DialogWindow) {
		...
		$("body").append(this.label
			...
			.click(function() {
				...
				self.scene.setView(self.name, function() {
					self.dialog = new DialogWindow(self.scene, "objectInfo", self.info, self.name, self.planet.name);
				});
				$(this).hide();
			})
		);
	...
	SatelliteObject.prototype.updatePosition = function(timestamp) {
		// takes a UNIX timestamp and positions the object for that date w.r.t. the body it's orbiting
